    ...
    for v in pending:
        with st.container(border=True):
            ...
            with cols[0]:
                ...
                number_part = f" · {v['uploader_student_number']}번" if v.get("uploader_student_number") else ""
                st.caption(f"{v['category']} · {v['uploader_name']} ({v.get('uploader_class') or '-'}){number_part}")
